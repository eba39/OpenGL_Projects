...
void createDisc()
{
	...
	for (i = 0; i < NumDiscPoints; ++i) 
	{
		...
		disc_points[disc_index][1] = -sin(theta);
		disc_points[disc_index][2] = 0.0f;
		disc_points[disc_index][3] = 1.0f;
		...
		if (i <= (NumDiscPoints - 2)) {
			...
			disc_indices[tIndices] = 0; 
			tIndices++;
			disc_indices[tIndices] = disc_index; 
			tIndices++;
			disc_indices[tIndices] = disc_index + 1; 
			tIndices++;
...
void updateDiscVertexNormals() {
	...
	//normalize normals
	for (i = 0; i < NumDiscPoints; i++)disc_normals[i] = normalize(vec3(disc_normals[i]));
}
