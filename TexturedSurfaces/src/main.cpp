...
GLfloat updatedAngle = 0.0f;
GLfloat angle_add = 2.0f;

GLuint planeTexID;
GLuint baseballTexID;
GLuint diceTexID;
...
void Initialize(void){
	...
	baseballTexID = loadTexture((char*)"baseball.jpg");
	diceTexID = loadTexture((char*)"dice.jpg");

	TexIDs[0] = loadTexture((char*)"moon.jpg");
	TexIDs[1] = loadTexture((char*)"aztec.jpg");
	TexIDs[2] = loadTexture((char*)"stop.jpg");
	TexIDs[3] = loadTexture((char*)"glass.jpg");
	TexIDs[4] = loadTexture((char*)"einstein.jpg");
	TexIDs[5] = loadTexture((char*)"cone.jpg");
...
void Display(void)
{
	...
	model = mat4(1.0f);
	model *= translate(mat4(1.0f), glm::vec3(-0.75f, -0.75f, 0.0f));
	model *= rotate(mat4(1.0f), radians(updatedAngle), vec3(1.0f, 0.5f, 0.0f));
	model *= scale(glm::mat4(1.0f), glm::vec3(0.5f, 0.5f, 0.5f));
	setMatrices();
	...
	model = mat4(1.0f);
	model *= translate(mat4(1.0f), glm::vec3(-0.0f, -0.75f, 0.0f));
	model *= rotate(mat4(1.0f), radians(updatedAngle), vec3(0.0f, 1.0f, 0.0f));
	model *= scale(glm::mat4(1.0f), glm::vec3(0.40f, 0.40f, 0.40f));
	setMatrices();
	...
	model *= translate(mat4(1.0f), glm::vec3(-0.75f, 0.75f, 0.0f));
	model *= rotate(mat4(1.0f), radians(updatedAngle), vec3(0.0f, -1.0f, 0.0f));
	model *= rotate(mat4(1.0f), radians(180.0f), vec3(0.0f, 0.0f, 1.0f));
	model *= scale(glm::mat4(1.0f), glm::vec3(0.40f, 0.40f, 0.40f));
	setMatrices();
...
void rotate_func(int i) {
	updatedAngle += angle_add;
	glutTimerFunc(30,rotate_func, 1);
	glutPostRedisplay();
}
...
int main(int argc, char** argv){
	...
	glutCreateWindow("Textured Surfaces      Author: Erik Almaguer");
